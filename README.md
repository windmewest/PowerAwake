...
1. 在 GitHub Releases 下载 `PowerAwake-Setup-1.1.0-win-x64.exe`。
2. 运行安装程序。程序安装到当前用户目录，不需要管理员权限。
3. 在任务栏通知区域找到无穷图标。右键可使用完整菜单；双击可快捷切换开关。

无需安装时，也可以下载 `PowerAwake-1.1.0-win-x64-portable.zip`。发布页同时提供 `SHA256SUMS.txt` 用于校验文件完整性。

### 发布者

创建并推送类似 `v1.1.0` 的 Git 标签后，仓库中的 GitHub Actions 会自动测试、构建安装包并创建 GitHub Release。
...
1. Download `PowerAwake-Setup-1.1.0-win-x64.exe` from GitHub Releases.
...
For a no-install option, download `PowerAwake-1.1.0-win-x64-portable.zip`. Each release also includes `SHA256SUMS.txt` for integrity verification.

### Publishing

Create and push a Git tag such as `v1.1.0`. The included GitHub Actions workflow tests the project, builds the installer, and creates a GitHub Release automatically.
...
- Incluye **Mantener pantalla encendida**, tiempos de atenuacion/apagado, brillo de atenuacion y atenuacion suave configurable.
...
1. Descarga `PowerAwake-Setup-1.1.0-win-x64.exe` desde GitHub Releases.
...
Como opcion sin instalacion, descarga `PowerAwake-1.1.0-win-x64-portable.zip`. Cada version tambien incluye `SHA256SUMS.txt` para verificar la integridad.

### Publicacion

Crea y envia una etiqueta de Git como `v1.1.0`. El flujo de GitHub Actions incluido prueba el proyecto, genera el instalador y crea automaticamente una version de GitHub Releases.
